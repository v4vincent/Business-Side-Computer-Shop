...
// make the add and remove buttons work
    ...
    public String buyProduct(@RequestParam("productID") Long productId, Model model) {
        ...
        String purchaseMessage = productService.buyProduct(productId);

        // Add the purchase message to the model for display
        model.addAttribute("purchaseMessage", purchaseMessage);

        // Redirect to the success or failure page
        if ("Purchase successful".equals(purchaseMessage)) {
